...
// 通常の S-MU2000.clap（REAPER などフル機能向け、ポート A-D の 4 本 + A/D
...
// foo_midi 向けはノートの口を 1 本だけ名乗る（パート 1-16 のみ）
#if defined(S_MU2000_FOOBAR)
...
private:
	...
	static bool note_port(uint32_t index, bool is_input, clap_note_port_info_t *info)
	{
		...
		// ノートイベント（event() にまだ残してある経路）を名乗る理由が無い
	...
	// ホストが同じノートを複数のノートポートへ同時に流してくることがある
	// （REAPER + yabridge で確認。CLAP には VST3 のバス接続のような「繋いで
	// いなければ届かない」仕組みが無く、ホストが全部の口へブロードキャスト
