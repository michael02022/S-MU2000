...
#
# foo_midi は CLAP を MIDI プレイヤーとして使うのに、ノートの入力ポートが
# ちょうど 1 本、音声の入力ポートが 0 本のものしか受け付けない
# （doc/clap-foobar.md）。通常の S-MU2000.clap とは口の形が違う別物として、
# 別の .o・別の ID・別のファイル名でビルドする
#
#   make clap-foobar          build/S-MU2000-foobar.clap を作る
#   make install-clap-foobar  それを CLAP の置き場へ複製する

CLAP_FOOBAR_BIN  := $(BUILD)/S-MU2000-foobar.clap
CLAP_FOOBAR_OBJS := $(BUILD)/clapfoobarobj/src/clap/plugin.o $(filter-out $(BUILD)/vst3obj/src/vst3/plugin.o,$(VST3_OBJS))

$(BUILD)/clapfoobarobj/%.o: %.cpp
	@mkdir -p $(dir $@)
	$(CXX) $(CXXFLAGS) -DS_MU2000_FOOBAR=1 $(CLAP_INC) $(IMGUI_FLAGS) -c -o $@ $<

clap-foobar: $(CLAP_FOOBAR_BIN)

$(CLAP_FOOBAR_BIN): $(OBJS) $(BUILD)/src/mu2000.o $(CLAP_FOOBAR_OBJS) $(PC_OBJS)
	@mkdir -p $(dir $@)
	$(CXX) $(CXXFLAGS) -shared -o $@ $^ $(LDFLAGS) -lwinmm -lole32 -lgdi32 -luser32 -lavrt -lcomdlg32 -lshell32 -ld3d11 -ldxgi -ld3dcompiler -ldwmapi -limm32

install-clap-foobar: $(CLAP_FOOBAR_BIN)
	mkdir -p "$(CLAP_INSTALL)"
	cp -f $(CLAP_FOOBAR_BIN) "$(CLAP_INSTALL)/"
	@echo "入れた: $(CLAP_INSTALL)/S-MU2000-foobar.clap"
...
.PHONY: all clean regen check test test-update vst3 install-vst3 probe clap install-clap clap-foobar install-clap-foobar au install-au au-probe check-au
