...
ノートの口が A-D の 4 本 + A/D INPUT）とは口の形が違う別物として、別の ID・別の
ファイル名でビルドしてある。
...
通常版は 4 本のノート入力（パート 1-16 / 17-32 / 33-48 / 49-64）と、サンプリング用の
A/D INPUT（音声入力ポート 1 本）を持っているため、このままでは foo_midi のプレイヤー
一覧に出てこない。`S_MU2000_FOOBAR` を立てたビルドでは:

* ノートの入力ポートを 1 本だけ名乗る（`src/clap/plugin.cpp` の `kPorts`）。
  パート 17-64（MIDI In B/C/D）はこの版では使えない — 通常の MIDI ファイルは
  16 チャンネルに収まるので実害は無い
...
（[doc/vst3.md](vst3.md) の「起動待ち」）。これは foo_midi 専用の問題では
なく、`activate()` が `m_engine.wait_ready()` で起動が終わるまで待ちきる
形で、通常版・foobar 向けの両方に共通して直してある（issue #19）。
`activate()` は音声スレッドではないので、ここで待っても実時間の音は落ちない。
foo_midi のように `Startup()` から戻ったらすぐ実物の MIDI を流し始めるホスト
（[Players/CLAPPlayer.cpp](https://github.com/stuerp/foo_midi/blob/main/Players/CLAPPlayer.cpp)
に latency extension を問い合わせる処理も、待つ処理も無い）でも、この版を
分ける理由にはならなかった。
